...
#define userappDEBUG_VERSION	3 ///局部修改或 bug 修正时+1
...
/*UserWakupWdg：复位外部看门狗
*看门狗复位时间：100ns延时
*参数：			 无
*返回值：   	 无
*/
void UserWakupWdg(void)
{
	ZetaHandle.PowerOff();
	
	for(uint16_t t = 0; t < 10; ++t)  //11.2us = 100 延时不能太高，zeta模块存在差异，会导致zeta模块重启
	__NOP();
